...
public class TopKElementsQuickSelect {
    ...
    public int partition(int left, int right, int pivot_index) {
        int pivot_frequency = recordMap.get(unique[pivot_index]);
        // 1. Move pivot to end
        ...
        for (int i = left; i <= right; i++) {
            if (recordMap.get(unique[i]) < pivot_frequency) {
                swap(store_index, i);
                store_index++;
            }
        }
